...
✅ Done Report stored at `.genie/reports/done-tests-<slug>-<YYYYMMDDHHmm>.md` with scenarios, commands, and follow-ups
...
3. [Verification]
   - Run targeted commands for Forge:
     • Rust: `cargo test --workspace -q` (all tests)
     • Rust specific: `cargo test -p <crate> <name> -q`
     • Frontend: `pnpm test`
...
### Integration Test (Rust - separate file)
```rust
...
- Execute agreed commands (cargo test, pnpm test) and copy relevant output into the report.
- Save the report at `.genie/reports/done-tests-<slug>-<YYYYMMDDHHmm>.md` (UTC) and link it in chat.
