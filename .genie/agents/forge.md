...
1. [Discovery]
   ...
   - Query Forge for project ID using `mcp__forge__list_projects` and select Automagik Forge project
   - Check for existing tasks with similar titles using `mcp__forge__list_tasks`
...
```markdown
## Task Overview
Implement Forge task management feature for automagik-forge.

## Context & Background
@crates/server/src/routes/tasks.rs — current task routes
@crates/db/src/models/task.rs — task data model
@crates/server/tests/task_integration.rs — baseline coverage

## Advanced Prompting Instructions
<context_gathering>
Goal: Inspect task routes, models, and test coverage for task management.
Method: Read referenced files; run targeted search if contracts unclear.
Early stop: Once current implementation and gaps are understood.
</context_gathering>

<task_breakdown>
1. [Discovery] Understand current task model and API contracts.
2. [Implementation] Add task management features with minimal disruption.
3. [Verification] Run `cargo test -p server test_task_management -q`.
</task_breakdown>

<SUCCESS CRITERIA>
✅ Task CRUD operations working correctly
✅ Existing API behaviour unchanged for other routes
✅ Tests documented and passing (command above)
</SUCCESS CRITERIA>

<NEVER DO>
❌ Modify frontend components (handled by another group)
❌ Write docs—note requirement instead
❌ Introduce non-Forge validation commands
</NEVER DO>
