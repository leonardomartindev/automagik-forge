...
// NOTE: This overlay performs a surgical fix for the MCP UI:
// - Rename the preconfigured server key from "automagik_forge" to "forge"
// - Point its icon to an existing asset so the logo is not broken
      ...
      try {
        ...
        if (pre && typeof pre === 'object') {
          ...
          if (pre.automagik_forge && !pre.forge) {
            pre.forge = pre.automagik_forge;
            delete pre.automagik_forge;
          }
          // Fix meta for display (name + icon)
          if (pre.meta) {
            const meta = pre.meta;
            if (meta.automagik_forge && !meta.forge) {
              meta.forge = { ...meta.automagik_forge };
              delete meta.automagik_forge;
            }
            if (meta.forge) {
              // Ensure friendly name and working icon path
              meta.forge.name = 'Forge';
              // Use an existing public asset to avoid broken logo
              meta.forge.icon = 'forge-dark.svg';
            }
          }
        }

        setMcpConfig(fixed as McpConfig);

        // Build full config for editor
